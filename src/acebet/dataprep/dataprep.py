"""Data preparation utilities for model training and inference datasets."""
...
def prepare_data() -> None:
    """Build and persist the production-ready ATP dataset.

    The transformation renames columns, balances player ordering, engineers
    time/ranking features, and writes the resulting table to a Feather file.
    """
    ...
    df.columns = df.columns.str.lower()
    df.rename(
        columns=lambda x: x.replace("winner", "p1").replace("loser", "p2"),
        inplace=True,
    )
